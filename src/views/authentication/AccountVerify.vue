<template>
  <container-v-align-no-background>
    <section v-if="!correoEnviado"
      ...
      <hr>
      <p>{{ $t("accountVerify.resendEmailMsg")}}</p>
      <button-submit v-if="!solicitandoEnvioDeCorreo" @button-submit="resendEmail">
...
export default {
  name: "AccountVerify",
  components: { IconCheck, ButtonSubmit, ContainerVAlignNoBackground },

  data() {
    return {
      solicitandoEnvioDeCorreo: false,
      correoEnviado:false
    }
  },
  methods: {
    resendEmail() {
      this.solicitandoEnvioDeCorreo = true;

      axios.get(import.meta.env.VITE_SERVICE_BASE_URL+"verificar-cuenta")
        .then(() => {
          this.correoEnviado = true;
          this.solicitandoEnvioDeCorreo = false
        })
        .catch(() => {
          this.solicitandoEnvioDeCorreo = false
        })

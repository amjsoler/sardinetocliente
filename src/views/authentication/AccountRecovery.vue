    ...
    <section v-if="!correoEnviado"
      ...
      <form class="space-y-4">
        <form-group>
          ...
          <email-input v-model="accountRecover.correo" />
          <small-error v-if="errors.correo">{{ errors.correo[0] }}</small-error>
        </form-group>
        <button-submit v-if="!procesando" @button-submit="restablecerCuenta">{{ $t("accountRecovery.sendMailBtnSubmit")}}</button-submit>
...
export default {
  ...
  components: { SmallError, SpanLabel, FormGroup, EmailInput, ButtonSubmit, ContainerVAlignNoBackground, IconCheck },
  data() {
    return {
      accountRecover: {
        correo: ""
      ...
      correoEnviado: false,
      procesando: false //TODO manejar esto de otra forma para no acoplar todos los componentes
    }
  },
  ...
  methods: {
    restablecerCuenta(){
      ...
      axios.post(import.meta.env.VITE_SERVICE_BASE_URL+"recuperar-cuenta", this.accountRecover)
        .then(() => {
